...
@section('content')
    <div class="d-flex align-items-center justify-content-between">
        <h5 class="mb-0 me-6">
            GERENCIAR PÁGINAS
            <a class="btn btn-primary btn-sm ms-6" style="padding: 2px 8px; font-size: 12px;" type="button" href="{{ route('pagina-create') }}">
        ...
        <nav aria-label="breadcrumb" class="mb-0">
            <ol class="breadcrumb mb-0">
                ...
                <li class="breadcrumb-item">
                    <a href="#" class="text-decoration-none fw-medium small">Gerenciar Paginas</a>
                </li>
    ...
    <div class="mt-4">
        <table id="example" class="table table-striped" style="width:100%">
            ...
            <tbody>
                ...
                <tr>
                    <td><img src="{{ asset('storage/' . $pagina->imagem) }}" alt="Imagem" style="width: 150px; height: 80px;"></td>   
                    <td>{{ $pagina->titulo }}</td>
                    <td>{{ $pagina->created_at->format('d/m/Y') }}</td>
                    <td>{{ $pagina->data_publicacao_formatada }}</td>
                    <td>{{ $pagina->publicado ? 'Sim' : 'Não' }}</td>
                    <td class="text-end">
                        <div class="d-inline">
                            <a href="{{  route('pagina-edit', ['id' => $pagina->id]) }}" class="btn btn-primary btn-sm ms-1" style="padding: 2px 8px; font-size: 12px;" type="button">
                                <i class="fas fa-pen me-1"></i> Editar
                            </a>
                            <form action="{{ route('pagina-delete', ['id' => $pagina->id]) }}" method="POST" style="display: inline;">
                                @csrf
                                @method('DELETE')
