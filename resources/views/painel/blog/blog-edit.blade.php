...
@section('content')
    <div class="d-flex align-items-center justify-content-between">
        ...
        <nav aria-label="breadcrumb" class="mb-0">
            <ol class="breadcrumb mb-0">
                ...
                <li class="breadcrumb-item">
                    <a href="#" class="text-decoration-none fw-medium small">Gerenciar Postagem do Blog</a>
                </li>
    ...
    <div class="card mt-3">
        ...
        <div class="card-body">
            <h5 class="card-title">Editar Postagem do Blog</h5>
        </div>

        <hr style="margin-top: -10px;" class="mb-4">
    
        <div class="mb-3 ms-3 me-3">
            <form action="{{ route('blog-update', ['blog' => $blog->id]) }}" method="POST" enctype="multipart/form-data" id="form">
                @csrf()
                <div class="row mb-3">
                    ...
                    <div class="col-md-2">
                        <img src="{{ asset('storage/' . $blog->imagem) }}" alt="Imagem" style="width: 150px; height: 80px;">
                    </div>
                </div>
                <div class="row mb-3">
                     <!-- Tipo de Postagem -->
                    <div class="col-md-6">
                        ...
                        <select class="form-select select2" name="tipo_postagem_id" id="tipo_postagem_id" required>
                            ...
                            @foreach ($tipoPostagens as $tipoPostagem)
                                <option value="{{ $tipoPostagem->id }}" {{ $tipoPostagem->id == $blog->tipo_postagem_id ? 'selected' : '' }}>{{ $tipoPostagem->tipo_postagem }}</option>
                            @endforeach
                        </select>
                    </div>
                    <!-- Categoria -->
                    <div class="col-md-6">
                        ...
                        <select class="form-select select2" name="categoria_id" id="categoria_id" required>
                            ...
                            @foreach ($categorias as $categoria)
                                <option value="{{ $categoria->id }}" {{ $categoria->id == $blog->categoria_id ? 'selected' : '' }}>{{ $categoria->nome }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                <div class="row mb-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <select class="form-select select2" name="autor_id" id="autor_id" required>
                            ...
                            @foreach ($autores as $autor)
                                <option value="{{ $autor->id }}" {{ $autor->id == $blog->autor_id ? 'selected' : '' }}>{{ $autor->autor }}</option>
                            @endforeach
                        </select>
                    </div>
                    <!-- Data Início -->
                    <div class="col-md-6">
                        ...
                        <input type="date" class="form-control" id="data_publicacao" name="data_publicacao" value="{{ $blog->data_publicacao }}" required>
                    </div>
                </div>
                <div class="row mb-3">
                    <!-- Fonte -->
                    <div class="col-md-6">
                        ...
                        <input type="text" class="form-control" id="fonte" name="fonte" value="{{ $blog->fonte }}" required>
                    </div>
                    <!-- Ativo -->
                    {{-- <div class="col-md-3 mt-4">
                        <input type="hidden" name="ativo" value="0">
                        <div class="form-check">
                            <input type="checkbox" class="form-check-input" id="ativo" name="ativo" value="1" {{ old('ativo', $blog->ativo ?? 0) ? 'checked' : '' }}>
                    ...
                    <div class="col-md-3 mt-4">
                        ...
                        <div class="form-check">
                            <input type="checkbox" class="form-check-input" id="publicado" name="publicado" value="1" {{ old('publicado', $blog->publicado ?? 0) ? 'checked' : '' }}>
    ...
    <script>
        ...
        @if(isset($blog->texto))
            console.log({!! json_encode($blog->texto) !!});
            quill.root.innerHTML = {!! json_encode($blog->texto) !!};
        @endif
